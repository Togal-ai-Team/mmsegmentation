...
def imflip(img: np.ndarray, direction: str = 'horizontal') -> np.ndarray:
    ...
    if direction == 'horizontal':
        ...
        return np.flip(img, axis=(1, 2))

@TRANSFORMS.register_module()
class RandomMultiMaskFlip(BaseTransform):
...
class RandomRotate(BaseTransform):
    """Rotate the image & seg to discrete angles.
    ...
    Args:
        ...
        angles (List[int], optional): List of discrete angles to rotate.
            Default: [90, 180, 270].
    ...
    def generate_angle(self):
        return np.random.rand() < self.prob, np.random.choice(self.angles)
    ...
    def transform(self, results: dict) -> dict:
        ...
        rotate, angle = self.generate_angle()
        if rotate:
            # rotate image
...
class PhotoMetricDistortion(BaseTransform):
    ...
    def __init__(self,
                 brightness_delta: int = 36,
                 contrast_range: Sequence[float] = (0.9, 1.1),
                 saturation_range: Sequence[float] = (1, 1),
                 hue_delta: int = 0):
            ...
            for i in range(n_holes):
                ...
                if self.seg_fill_in is not None:
                    for key in results.get('seg_fields', []):
                        ...
                        for i in range(seg_map.shape[0]):
                            seg_map[i, y1:y2, x1:x2] = self.seg_fill_in
                        results[key] = seg_map
